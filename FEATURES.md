...
| **Runtime defaults endpoint** | `web/app/api/marketplace/settings/defaults/route.ts` exposes authenticated read-only marketplace defaults (`commission`, `deposit`, `quote-deadline-hours`) for non-platform operational flows. | Allows runtime marketplace UX (award/event posting flows) to consume the latest platform-configured defaults without requiring platform-admin privileges. |
...
| **Active runtime wiring (`web-marketplace`)** | `web-marketplace/components/marketplace/award/AwardConfirmationModal.tsx` now waits for defaults fetch before first deposit initialization; `web-marketplace/stores/useEventPostingStore.ts` hydrates quote-deadline defaults and is invoked from `web-marketplace/app/events/create/page.tsx` and `web-marketplace/app/events/[id]/edit/page.tsx`. | Ensures operator changes to deposit/deadline defaults are reflected in the currently mounted marketplace runtime user flows. |
...
- **CompanyProfileCard**: Company header card with 5-stat grid (Team Size, Rating with star icon, Events Completed, Insurance status badge, Coverage areas). Verified badge shown for verified/cqc_verified companies. Member since date. Insurance badge colours: green (verified), red (expired), gray (unverified). Events completed count derived from awarded marketplace quotes on completed events.
...
- **MedicAvailabilityModal**: Dialog for company admin to set/clear medic date-range unavailability. Opened via the Edit button on RosterMedicCard. "Unavailable from" and "Unavailable until" date inputs, optional reason text. "Clear availability" button restores availability. PATCH `/api/marketplace/roster/[id]` with availability fields. Pre-populates form with existing dates. Sonner toast + query cache invalidation on success.
