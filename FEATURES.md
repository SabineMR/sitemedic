...
| **Award + event wizard wiring** | `web/components/marketplace/award/AwardConfirmationModal.tsx` now waits for defaults fetch before first deposit initialization; `web/stores/useEventPostingStore.ts` + `web/components/marketplace/event-wizard/ScheduleLocationStep.tsx` hydrate quote-deadline defaults from settings endpoint. | Closes first-open timing gaps so operator changes to deposit/deadline defaults are reflected reliably in user-facing runtime flows. |
