# Requirements Archive: v3.0 White-Label Platform & Subscription Engine
...
- [x] **INFRA-01**: Next.js is updated to ≥15.2.3 (CVE-2025-29927 patch required before subdomain routing expands the attack surface)
- [x] **INFRA-02**: `org_branding` table created (logo_path, primary_colour_hex, company_name, tagline) with RLS policies scoped to org_id
- [x] **INFRA-03**: `organizations` table gains subscription columns: `stripe_customer_id`, `stripe_subscription_id`, `subscription_tier` (starter/growth/enterprise), `subscription_status` (active/past_due/cancelled)
- [x] **INFRA-04**: `org-logos` Supabase Storage bucket created as public with org_id-namespaced paths (public URLs required — logos appear in PDFs and emails hours after generation)
- [x] **INFRA-05**: Existing orgs are backfilled with default subscription tier (growth for Apex Safety Solutions, starter for all others) and empty org_branding rows
...
- [x] **BRAND-03**: Org branding is applied to the web portal — header logo, sidebar accent colour, login page company name and logo, browser tab title
- [x] **BRAND-04**: Org logo and company name appear in the header of all generated PDFs (weekly reports, RIDDOR F2508, payslips, invoices) — replaces hardcoded SiteMedic identity
- [x] **BRAND-05**: Org logo and primary colour appear in all transactional emails (booking confirmations, shift alerts, invoice notifications) — replaces hardcoded email branding
- [x] **BRAND-06**: Branding is resolved server-side in Next.js middleware via `x-org-*` request headers — zero client-side branding fetches on page load

### ROUTE — Subdomain Routing

- [x] **ROUTE-01**: Each org on Growth or Enterprise tier is accessible at `slug.sitemedic.co.uk` (e.g. `apex.sitemedic.co.uk`) — Vercel wildcard domain `*.sitemedic.co.uk` configured
- [x] **ROUTE-02**: Next.js middleware extracts the subdomain from the `host` header, looks up the org by slug using service role, and injects org context as `x-org-*` request headers
- [x] **ROUTE-03**: Incoming `x-org-*` headers are stripped at the top of middleware before being re-set (prevents external header injection)
- [x] **ROUTE-04**: The branded login page at `slug.sitemedic.co.uk/login` shows the org's logo and company name — unauthenticated visitors see the org's brand, not SiteMedic's
...
**Shipped:** 30 of 30 v3.0 requirements
