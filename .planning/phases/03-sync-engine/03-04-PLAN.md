...
must_haves:
  truths:
    ...
    - "RIDDOR-reportable incident that fails to sync triggers a critical persistent banner after 3+ retry attempts"
  ...
  artifacts:
    ...
    - path: "src/components/RiddorSyncAlert.tsx"
      provides: "Persistent critical banner for RIDDOR sync failures after retry threshold"
  ...
  key_links:
    ...
    - from: "src/components/RiddorSyncAlert.tsx"
      ...
      via: "checks for failed RIDDOR items in sync queue with retryCount >= 3"
      pattern: "retryCount.*>=.*3|RIDDOR_RETRY_THRESHOLD"
...
This component checks the sync queue for RIDDOR items (priority 0, excluding photo_uploads) that have **genuinely failed to sync** -- meaning they have retried at least 3 times (`retryCount >= 3`). This threshold ensures normal transient retries (30s, 1min, 2min backoff) do not trigger the critical alert, while sustained failures (failing for several minutes across multiple retry cycles) do.

IMPORTANT: The threshold is `retryCount >= 3` (the `RIDDOR_RETRY_THRESHOLD` constant). Rationale:
- retryCount 1: first retry after ~30s backoff -- likely transient network blip
- retryCount 2: second retry after ~1min backoff -- still could be brief outage
- retryCount 3: third retry after ~2min backoff -- item has been failing for ~3.5 minutes total, warrants critical alert

If any RIDDOR items exceed this threshold, a NON-DISMISSIBLE red banner appears at the top of the screen.
...
- RiddorSyncAlert: shows ONLY for RIDDOR items that have failed >= 3 retry attempts, red, non-dismissible until resolved
...
Grep for RIDDOR threshold: `grep -n "RIDDOR_RETRY_THRESHOLD\|retryCount.*>=.*3" src/components/RiddorSyncAlert.tsx`
  </verify>
  <done>
SyncErrorDisplay shows plain English errors with retry button for general sync failures. RiddorSyncAlert shows persistent red banner for RIDDOR items that have failed to sync after 3+ retry attempts (retryCount >= RIDDOR_RETRY_THRESHOLD), checking queue every 10 seconds. Normal transient retries (1-2 attempts) do NOT trigger the critical alert.
  </done>
</task>
...
- RiddorSyncAlert checks sync queue for priority 0 items with retryCount >= 3 (RIDDOR_RETRY_THRESHOLD)
- RiddorSyncAlert does NOT trigger on first or second retry (normal transient failures)
...
2. RIDDOR sync failures show persistent red banner (non-dismissible) with "Sync Now" button ONLY after 3+ failed retry attempts (not on first retry)
