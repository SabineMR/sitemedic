...
Grep for RIDDOR priority: `grep -rn "isRiddorReportable.*0.*1\|priority.*0" mobile/app/treatment/`
  ...
  <done>
All Phase 2 data capture forms (treatments, near-misses, workers, daily checks) enqueue sync operations after local WatermelonDB writes. RIDDOR treatments use priority 0. Photos queued via photoUploadQueue for progressive upload.
  </done>
</task>
...
Verify background task import at top: `head -10 App.tsx`
  ...
  <done>
App.tsx imports background task at global scope, mounts RiddorSyncAlert, SyncErrorDisplay, and PhotoUploadProgress inside SyncProvider above existing navigation content.
  </done>
</task>
