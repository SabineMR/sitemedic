...
must_haves:
  truths:
    ...
    - "On the treatments list, any motorsport head injury with concussion suspected and no competitor_stood_down shows a red 'Concussion clearance required' badge"
  ...
  key_links:
    ...
    - from: "web/components/dashboard/treatments-columns.tsx"
      ...
      pattern: "concussion_suspected.*competitor_stood_down"
---
...
<task type="auto">
  ...
  <action>
    ...
    **D. `index.ts`:**
    ...
    Note: This function uses the same `motorsport-reports` storage bucket created by migration 125 (Plan 19-03). If Plan 19-03 runs before or in parallel, the bucket will exist. If not, the upload will fail gracefully with an error response.
  </action>
  <verify>
...
<task type="auto">
  ...
  <action>
    **A. Add concussion clearance badge to `treatments-columns.tsx`:**
    ...
    ```typescript
    {
      ...
      cell: ({ row }) => {
        ...
        if (
          ...
          extraFields.competitor_stood_down !== true
        ) {
          return (
    ...
    Critical null-safety: `extraFields` must be checked for null before accessing nested properties. Pre-Phase-18 treatments have `null` `vertical_extra_fields`. Use `!== true` (not `=== false`) to handle undefined safely.
  ...
  <verify>
    ...
    3. The badge renders ONLY when `event_vertical === 'motorsport'` AND `concussion_suspected === true` AND `competitor_stood_down !== true`.
  ...
  <done>
    The treatments table shows a red "Concussion clearance required" badge for any motorsport treatment with suspected concussion that has not been cleared. The booking detail page has a "Generate Medical Statistics Sheet" button for motorsport bookings that generates and downloads the aggregate PDF.
  </done>
</task>
...
2. The concussion badge column renders correctly: shows badge for uncleared concussion, nothing for cleared or non-motorsport.
...
- Concussion badge appears on treatments table for uncleared motorsport concussions
