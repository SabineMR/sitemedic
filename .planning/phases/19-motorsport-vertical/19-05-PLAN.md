...
**Human checkpoint dependency:** Plan 03 (Wave 2) contains a `checkpoint:human-action` task (Task 1: Download Motorsport UK Incident Pack V8.0) that must be resolved by a human before Plan 03 completes. Since Plan 04 (Wave 3) depends on Plan 03, and this Plan 05 (Wave 4) depends on Plan 04, execution of Waves 3 and 4 is blocked until the human responds to the Plan 03 checkpoint. The executor must wait for the Plan 03 checkpoint to be resolved before proceeding to Plan 04 and then Plan 05.
...
Note: Wave 3 (Plan 04) cannot begin until a human has responded to the Plan 03 Task 1 checkpoint. Wave 4 (this plan) cannot begin until Wave 3 completes.
</context>
...
<task type="auto">
  ...
  <action>
    ...
    **SC-1: Motorsport form fields + concussion gate (MOTO-01, MOTO-03)**
    - Read `app/treatment/new.tsx` and confirm:
      ...
      - `event_vertical` included in sync payload (set from orgVertical)
      - A `motorsport_concussion` alert is inserted into `medic_alerts` when concussion is suspected
    ...
    **SC-3: Dashboard concussion badge (MOTO-03)**
    - Read `web/components/dashboard/treatments-columns.tsx` and confirm:
      ...
      - Badge renders when `concussion_suspected === true && competitor_cleared_to_return !== true`
  ...
  <done>
    All phase success criteria verified: motorsport form fields render correctly, concussion gate blocks submission, RIDDOR is disabled, dashboard badge shows for uncleared concussions (competitor_cleared_to_return !== true), Medical Statistics Sheet generates, cert profile ordering is correct, and motorsport_concussion alert is created.
  </done>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>
    ...
    3. Concussion admin notification — motorsport_concussion alert inserted into medic_alerts when concussion treatment is submitted
    4. Motorsport PDF Edge Function — generates Motorsport UK Accident Form PDF per incident (with "Event Organiser" label per MOTO-05)
    5. Medical Statistics Sheet — aggregate PDF per booking with treatment counts, severity breakdown, GCS range, concussion/extrication counts
    6. Dashboard concussion badge — red "Concussion clearance required" badge on treatments table for uncleared motorsport concussions (competitor_cleared_to_return !== true)
    7. Cert types — Motorsport UK Medical Official Licence and BASM Diploma added, ordering: Medical Official Licence, HCPC Paramedic, PHTLS first
    8. RIDDOR gate — verified motorsport treatments never trigger RIDDOR
  </what-built>
  <how-to-verify>
...
3. Dashboard concussion badge visible for uncleared cases (competitor_cleared_to_return !== true)
4. Medical Statistics Sheet generates from booking detail
5. Cert selector shows correct ordering for motorsport
6. Motorsport concussion alert created in medic_alerts (MOTO-03 admin notification)
</success_criteria>
