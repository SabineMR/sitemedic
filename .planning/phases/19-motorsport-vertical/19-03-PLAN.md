...
MOTO-05 "Organiser" terminology note: The research confirms "Organiser" appears in the PDF context as the entity that receives the Medical Statistics Sheet (MOTO-02) and is referenced in regulatory text. In this plan, the PDF includes an "Event Organiser" label in the event details section (mapped from `org_name`). The existing taxonomy already uses correct terminology for "Competitor" (`getPatientLabel('motorsport')` returns `'Driver / Competitor'`) and "Clerk of Course" (in extra fields). No additional taxonomy changes are needed for MOTO-05.
...
<task type="auto">
  ...
  <action>
    ...
    **B. Update `types.ts`:**
    ...
    export interface MotorsportFormData {
      ...
      event_date: string;
      venue: string;
      org_name: string;  // "Event Organiser" label in PDF (MOTO-05)
    ...
    **C. Create `motorsport-mapping.ts`:**
    ...
    In the mapping, use `org_name` as the "Event Organiser" label value (this satisfies MOTO-05 — the organiser is the organization that booked the event).
    ...
    **D. Create `MotorsportIncidentDocument.tsx`:**
    ...
    - Layout: A4 page with header "Motorsport Incident Report", event details section (including "Event Organiser: {org_name}" label per MOTO-05), competitor details section, clinical assessment section (including GCS score), motorsport-specific section (extrication, helmet, circuit section), concussion clearance section (if applicable), outcome section, and footer with reference number and generation timestamp.
  ...
  <verify>
    ...
    6. The PDF event details section includes "Event Organiser" label (MOTO-05).
  </verify>
  <done>
    The `motorsport-incident-generator` Edge Function generates a Motorsport UK-style Accident Form PDF for any motorsport treatment. The PDF includes all MOTO-01 fields plus clinical data, with "Event Organiser" label for MOTO-05. The `motorsport-reports` storage bucket is created by migration 125. The function returns a 7-day signed URL for PDF download.
  </done>
</task>
...
6. "Event Organiser" label appears in the PDF event details section (MOTO-05 validated).
</verification>
...
- PDF includes "Event Organiser" label in event details section (MOTO-05)
