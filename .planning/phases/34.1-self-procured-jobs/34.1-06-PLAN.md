...
must_haves:
  truths:
    ...
    - "POST /api/direct-jobs succeeds when the company's org has an active subscription (subscription_status='active' or NULL for legacy orgs)"
...
After confirming the user is a company admin, query the company's `org_id` from marketplace_companies (add `org_id` to the existing select), then query `organizations` to check `subscription_tier` is not null and `subscription_status` is not 'past_due' or 'cancelled'. Per migration 133 convention, NULL subscription_status means "legacy org not yet on Stripe billing — treat as active".
...
After the job query, query bookings with `source='direct'` matching by site_name + site_postcode + org_id. Also match shift_date to the first event day for additional reliability. Add a TODO comment noting a proper FK link should be added in a future migration.

```ts
// Check payment status via booking existence
// TODO: Add marketplace_events.id FK on bookings table for reliable reverse lookup (current approach uses name+postcode+org matching)
...
const firstDay = ((job.event_days || []) as Array<{ event_date: string }>)
...
if (company?.org_id) {
  bookingQuery.eq('org_id', company.org_id);
}
if (firstDay) {
  bookingQuery.eq('shift_date', firstDay.event_date);
}

const { data: relatedBooking } = await bookingQuery.maybeSingle();
...
5. Grep for "bookings" in web/app/api/direct-jobs/[id]/client-access/route.ts to confirm payment wiring exists
  ...
  <done>
- POST /api/direct-jobs returns 403 when company has no org_id, no subscription_tier, or subscription_status is 'past_due'/'cancelled'
- POST /api/direct-jobs succeeds for companies with subscription_tier set and subscription_status='active' or NULL (legacy orgs per migration 133)
