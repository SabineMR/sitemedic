...
<task type="auto">
  <name>Task 1: Create contract PDF types, styles, and main ContractDocument component</name>
  <files>
    supabase/functions/generate-contract-pdf/types.ts
    supabase/functions/generate-contract-pdf/styles.ts
    supabase/functions/generate-contract-pdf/components/ContractDocument.tsx
...
- Import and render ServiceTerms, PaymentSchedule, SignatureBlock sub-components (created in Task 2)
...
- GBP currency formatting: £1,000.00 with toLocaleString('en-GB')
  ...
  <verify>
Run: `ls supabase/functions/generate-contract-pdf/` to verify types.ts, styles.ts exist.
Run: `ls supabase/functions/generate-contract-pdf/components/` to verify ContractDocument.tsx exists.
...
Run: `grep "Document" supabase/functions/generate-contract-pdf/components/ContractDocument.tsx` confirms Document import.
  ...
  <done>
3 files created: types.ts with ContractPDFData interface, styles.ts with brand-consistent React-PDF StyleSheet, ContractDocument.tsx composing all PDF sections with proper UK date/currency formatting.
  </done>
</task>

<task type="auto">
  <name>Task 2: Create ServiceTerms, PaymentSchedule, and SignatureBlock sub-components</name>
  <files>
    supabase/functions/generate-contract-pdf/components/ServiceTerms.tsx
    supabase/functions/generate-contract-pdf/components/PaymentSchedule.tsx
    supabase/functions/generate-contract-pdf/components/SignatureBlock.tsx
...
- Include IP address note: "Digital signature captured and stored with timestamp and IP address for verification."
  ...
  <verify>
Run: `ls supabase/functions/generate-contract-pdf/components/` to verify ServiceTerms.tsx, PaymentSchedule.tsx, SignatureBlock.tsx exist.
Run: `grep "clauses" supabase/functions/generate-contract-pdf/components/ServiceTerms.tsx` confirms clause rendering.
Run: `grep "PaymentSchedule" supabase/functions/generate-contract-pdf/components/PaymentSchedule.tsx` confirms component.
  </verify>
  <done>
3 sub-components created: ServiceTerms renders numbered clauses and legal text. PaymentSchedule renders milestone table for all 5 term types. SignatureBlock shows dual-column provider/client signature areas with signed/unsigned states.
  </done>
</task>

<task type="auto">
  <name>Task 3: Create Edge Function handler with PDF generation endpoint</name>
