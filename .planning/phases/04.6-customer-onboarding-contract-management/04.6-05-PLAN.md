...
- Check Edge Function response for `success: false` (graceful degradation when RESEND_API_KEY missing):
  - Parse the response JSON from `supabase.functions.invoke`
  - If `data.success === false`, return error response to client: `{ success: false, error: 'Email service not configured. Contract created but email could not be sent.', shareableLink }` with HTTP 502
  - The contract status should NOT be updated to 'sent' in this case (remains in current status since email was not delivered)
  - Still return the shareableLink so admin can share it manually during the phone call
- On successful Edge Function response:
