...
<task type="auto">
  ...
  **Summary of changes:**
  ...
  2. **CRITICAL:** Run `grep -c 'company_name' supabase/functions/certification-expiry-checker/index.ts` -- MUST return 0 (zero matches). All `company_name` references must be replaced with `name`.
  ...
  6. Verify no residual company_name: `grep 'company_name' supabase/functions/certification-expiry-checker/index.ts` must produce NO output
  </verify>
  <done>
  ...
  - ZERO instances of 'company_name' remain in the file (all replaced with 'name')
  </done>
</task>
...
<verification>
- `grep -c "company_name" supabase/functions/certification-expiry-checker/index.ts` returns 0 (ZERO -- this is the critical check)
...
- `grep "company_name" supabase/functions/certification-expiry-checker/index.ts` produces NO output (empty result)
</verification>
...
5. `grep -c 'company_name' supabase/functions/certification-expiry-checker/index.ts` returns exactly 0
</success_criteria>
