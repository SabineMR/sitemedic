...
<task type="auto">
  ...
  **B. Timesheet batch approval (`web/components/admin/timesheet-batch-approval.tsx`):**
  ...
  This is a client component. Import `exportTimesheetsCSV` from `@/lib/utils/export-csv`. The component receives `initialData` as a prop. Add an export button in the component header area. Find the section where the payout summary / action buttons are rendered and add:

  ```tsx
  <Button
    variant="outline"
    size="sm"
    onClick={() => exportTimesheetsCSV(data)}
  ...
  Where `data` is the timesheets array used by the table. Import `Download` from `lucide-react` (may already be imported as `FileText` is there).
  ...
  **D. Revenue page (`web/app/admin/revenue/page.tsx`):**
  ...
  Import `createClient` from `@/lib/supabase/client`. Get `orgId` from `useRequireOrg()` -- check if already available. If `useRevenue` uses `useRequireOrg`, import it. Also need `getDateRange` -- either import from revenue.ts or inline the simple date subtraction.

  Actually, simpler: import `useRequireOrg` from `@/contexts/org-context` (the revenue hook already uses it). For `getDateRange`, it is not exported from revenue.ts. Inline the logic:
  ```typescript
