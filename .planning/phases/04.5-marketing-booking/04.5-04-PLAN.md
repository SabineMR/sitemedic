...
must_haves:
  ...
  artifacts:
    - path: "web/app/api/bookings/match/route.ts"
      provides: "POST endpoint calling auto-assign Edge Function and returning ranked matches"
      exports: ["POST"]
  ...
  key_links:
    - from: "web/app/(booking)/book/confirmation/page.tsx"
      to: "web/app/api/bookings/match/route.ts"
      via: "Fetches medic matches after booking confirmed"
      pattern: "api/bookings/match"
...
<task type="auto">
  ...
  <action>
    ...
    6. **web/app/api/bookings/match/route.ts** - POST endpoint for auto-matching:
       - Accepts: `{ bookingId: string }`
       - Calls the existing auto-assign-medic-v2 Edge Function via Supabase client: `supabase.functions.invoke('auto-assign-medic-v2', { body: { booking_id: bookingId } })`
       - The Edge Function returns the assigned medic and match details
       ...
       - If auto-assignment fails (no available medics, score < 50), return: `{ matches: [], requiresManualApproval: true, reason: "..." }`
       - ALSO: After successful match, trigger email sending by calling POST /api/email/booking-confirmation internally

    7. **web/app/api/email/booking-confirmation/route.ts** - POST endpoint for sending confirmation emails:
       - Accepts: `{ bookingId: string }`
       - Fetches booking, client, and medic data from database
  ...
  <done>
    Auto-match API calls existing Edge Function and formats results for UI. Calendar invite generator produces valid .ics strings. Resend client initialized with dev fallback. Email templates created for client (with .ics attachment) and medic (assignment notification). Recurring booking endpoint creates child booking records. All files compile without errors.
  </done>
</task>
  ...
  <action>
    ...
    4. **web/app/(booking)/book/confirmation/page.tsx** - Confirmation page:
       ...
       - On mount:
         ...
         b. Call POST /api/bookings/match with bookingId to trigger auto-matching
    ...
    IMPORTANT: The confirmation page handles the full post-payment flow: verify payment -> create booking record (if not already created) -> auto-match medic -> send emails -> display confirmation. This is the critical path where everything connects.
  </action>
  <verify>
...
8. POST /api/bookings/match calls the existing auto-assign-medic-v2 Edge Function
9. POST /api/email/booking-confirmation sends emails with .ics attachment
10. Graceful handling when no medics available (manual approval message)
</verification>

<success_criteria>
- Auto-matching calls existing Edge Function and presents results with transparency
