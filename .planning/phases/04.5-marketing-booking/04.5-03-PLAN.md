...
must_haves:
  truths:
    ...
    - "New client can register inline (email, company, phone) before payment"
  artifacts:
    - path: "web/app/api/bookings/create-payment-intent/route.ts"
      provides: "POST endpoint creating booking with status='pending' AND Stripe Payment Intent"
      exports: ["POST"]
    - path: "web/app/api/bookings/create/route.ts"
      provides: "POST endpoint creating booking record in database for Net 30 clients"
      exports: ["POST"]
    ...
    - path: "web/components/booking/client-registration-form.tsx"
      provides: "Inline registration form for new clients"
      contains: "signUp"
  ...
  key_links:
    - from: "web/components/booking/payment-form.tsx"
      ...
      via: "fetch to create booking + Payment Intent, receives clientSecret"
      pattern: "create-payment-intent"
    - from: "web/app/api/stripe/webhooks/route.ts"
      to: "bookings table"
      via: "Updates booking status from 'pending' to 'confirmed' on payment_intent.succeeded"
      pattern: "payment_intent.succeeded"
    - from: "web/components/booking/client-registration-form.tsx"
      to: "web/lib/supabase/client.ts"
      via: "Supabase auth.signUp() + clients table insert"
      pattern: "signUp"
...
@web/lib/booking/types.ts (BookingFormData from Plan 02)
</context>
...
<task type="auto">
  ...
  <action>
    ...
    4. **web/app/api/bookings/create/route.ts** - POST endpoint to create a booking record (used by Net 30 flow):
       ...
       - Creates booking record in the `bookings` table via Supabase server client
    ...
    5. **web/app/api/bookings/create-payment-intent/route.ts** - POST endpoint for prepay clients that creates BOTH the booking record AND the Stripe Payment Intent in one call:
       - Accepts: BookingFormData + clientId
       - **Step 1:** Server-side pricing validation (same as /create endpoint)
       - **Step 2:** Create booking record in `bookings` table with status='pending' (NOT 'confirmed' -- payment hasn't happened yet)
       - **Step 3:** Fetch client from database to get stripe_customer_id (or create Stripe customer if none exists)
       - **Step 4:** Create Stripe Payment Intent:
         ...
         ```
       - Returns: `{ bookingId: string; clientSecret: string }`
       - Error: 400 if validation fails, 500 for Stripe errors
       - CRITICAL SEQUENCE: Booking must be created with status='pending' BEFORE Payment Intent. The webhook (payment_intent.succeeded) will update the booking from 'pending' to 'confirmed'. This prevents creating bookings without payment and prevents the email endpoint from finding a booking without a medic_id.
    ...
    6. **web/app/api/stripe/webhooks/route.ts** - Stripe webhook handler:
       ...
       - Handle events:
         - `payment_intent.succeeded`: Update booking status from 'pending' to 'confirmed', update client total_bookings count
  ...
  <done>
    Stripe client and server utilities created. Three API routes operational: POST /api/bookings/create (creates booking for Net 30 clients with server-side price validation), POST /api/bookings/create-payment-intent (creates booking with status='pending' + Stripe Payment Intent for prepay clients), POST /api/stripe/webhooks (handles payment success/failure, updates booking from 'pending' to 'confirmed'). All routes compile without TypeScript errors.
  </done>
</task>

<task type="auto">
  <name>Task 2: Create client registration form, payment page with Stripe Payment Element and Net 30 confirmation</name>
  <files>
    web/components/booking/client-registration-form.tsx
    web/components/booking/payment-form.tsx
    web/components/booking/net30-confirmation.tsx
  ...
  <action>
    1. **web/components/booking/client-registration-form.tsx** - 'use client' component for new client registration:
       - Inline form with fields: Email (required, email type), Company Name (required), Contact Name (required), Phone (required)
       - On submit:
         a. Call Supabase `auth.signUp({ email, password: auto-generated })` to create auth account (send magic link for password setup later)
         b. Insert record into `clients` table: `{ company_name, contact_name, email, phone, payment_terms: 'prepay', credit_limit: 0 }`
         c. Return the new client_id
       - Shows loading spinner during registration
       - Error handling: "Email already registered" -> show login link, other errors -> display message
       - Props: `{ onRegistered: (clientId: string, paymentTerms: 'prepay' | 'net_30') => void }`
       - Uses shadcn/ui Input and Button components

    2. **web/components/booking/payment-form.tsx** - 'use client' component with Stripe Payment Element:
       - Wraps content in Elements provider from @stripe/react-stripe-js with stripePromise and clientSecret
       - On mount: reads booking data from sessionStorage, calls POST /api/bookings/create-payment-intent (which creates BOTH the booking record with status='pending' AND the Payment Intent) to get clientSecret and bookingId
       - Shows loading spinner while booking + Payment Intent are being created
       ...
       - IMPORTANT: PaymentForm does NOT call /api/bookings/create separately. The create-payment-intent endpoint handles booking creation. This prevents creating bookings without payment.
    ...
    3. **web/components/booking/net30-confirmation.tsx** - 'use client' component for Net 30 clients:
       ...
       - "Confirm Booking" button that calls POST /api/bookings/create with net_30 flag (creates booking with status='confirmed' directly)
    ...
    4. **web/app/(booking)/book/payment/page.tsx** - Payment page:
       - 'use client' component (needs sessionStorage access and dynamic client state)
       - On mount, reads booking form data from sessionStorage (set by booking-form.tsx in Plan 02)
       - Determines payment flow:
         a. Check Supabase auth session. If not authenticated, show ClientRegistrationForm first.
         b. After registration or if already authenticated, fetch client record to check payment_terms.
         c. For NEW clients (just registered, payment_terms='prepay'): Show PaymentForm
         d. For returning clients with payment_terms='prepay': Show PaymentForm
         e. For returning clients with payment_terms='net_30': Show Net30Confirmation
  ...
  <verify>
    Run `pnpm build` from web/ directory. Navigate to http://localhost:30500/book/payment. Verify: page renders (may show registration form if not authenticated, or loading state if no booking data in sessionStorage). Check that ClientRegistrationForm component uses Supabase auth.signUp. Check that PaymentForm component imports @stripe/react-stripe-js correctly. Check that Net30Confirmation component renders booking summary.
  </verify>
  <done>
    Payment page at /book/payment renders ClientRegistrationForm for unauthenticated users, PaymentForm for prepay clients (Stripe Payment Element with 3D Secure), and Net30Confirmation for established clients. Client registration creates Supabase auth account + clients record with payment_terms='prepay'. PaymentForm calls create-payment-intent which creates booking (status='pending') + Payment Intent in one call. Webhook updates booking to 'confirmed' on payment success. All components compile without errors.
  </done>
</task>
...
3. Unauthenticated users see ClientRegistrationForm with email, company, contact, phone fields
4. After registration, PaymentForm renders with Stripe Payment Element (requires NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY env var)
5. POST /api/bookings/create-payment-intent creates booking (status='pending') AND Payment Intent in one call
6. POST /api/bookings/create accepts booking data for Net 30 clients and returns bookingId with status='confirmed'
7. POST /api/stripe/webhooks verifies Stripe signature and updates booking from 'pending' to 'confirmed'
8. Net 30 confirmation shows booking summary without payment form
9. Back link to /book works
</verification>

<success_criteria>
- ClientRegistrationForm creates auth account + client record via Supabase signUp
...
- Booking created with status='pending' before payment, webhook updates to 'confirmed'
