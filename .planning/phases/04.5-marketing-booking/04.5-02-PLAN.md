...
must_haves:
  truths:
    ...
    - "Client can toggle recurring booking with weekly/biweekly pattern and week count"
  artifacts:
    ...
    - path: "web/components/booking/location-input.tsx"
      ...
      contains: "postcode"
    - path: "web/components/booking/shift-config.tsx"
      provides: "Shift times, special requirements, and recurring booking UI"
  ...
  key_links:
    ...
    - from: "web/components/booking/shift-config.tsx"
      to: "web/lib/booking/types.ts"
      via: "Recurring fields bound to BookingFormData.isRecurring/recurrencePattern/recurringWeeks"
      pattern: "isRecurring"
...
<task type="auto">
  ...
  <action>
    ...
    3. **web/components/booking/shift-config.tsx** - 'use client' component.
       ...
       - **Recurring booking section (MUST be explicitly rendered):**
         - Checkbox with label "Recurring booking" that toggles formData.isRecurring
         - When isRecurring is checked, show two additional inputs:
           a. Select dropdown for recurrence pattern with options: "Weekly" (value: 'weekly') and "Biweekly" (value: 'biweekly'), bound to formData.recurrencePattern
           b. Number input with label "Number of weeks" (min=1, max=52), bound to formData.recurringWeeks
         - When isRecurring is unchecked, hide the pattern select and weeks input, and reset recurrencePattern to null and recurringWeeks to 0
       - Props: `{ formData: BookingFormData; onChange: (updates: Partial<BookingFormData>) => void }`
    ...
    IMPORTANT: Use shadcn/ui components (Input, Button, Select, Card, Checkbox) for all form elements for visual consistency with the existing dashboard.
  </action>
  <verify>
    Run `pnpm build` from web/ directory. Navigate to http://localhost:30500/book. Verify: calendar picker shows and allows date selection, location fields accept input, shift config calculates hours, recurring booking checkbox toggles pattern and weeks inputs, pricing breakdown updates in real-time as form values change, "Continue to Payment" button appears.
  </verify>
  <done>
    Booking page at /book renders with calendar picker, location input, shift configuration (including recurring booking toggle with pattern select and weeks input), and live pricing breakdown. All form inputs work. Urgency premium auto-calculates based on selected date. Pricing updates in real-time. "Continue to Payment" button navigates to /book/payment. Layout uses shared SiteHeader from Plan 01.
  </done>
</task>
...
11. Recurring booking checkbox shows/hides pattern select and weeks input
12. Selecting "Weekly" and entering 4 weeks stores correct values in form state
</verification>
...
- Recurring booking toggle with weekly/biweekly options and weeks input visible when checked
