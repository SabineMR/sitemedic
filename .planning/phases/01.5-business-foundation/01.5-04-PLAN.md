...
<task type="auto">
  <name>Task 1: Generate UK postcode sector dataset (reference CSV)</name>
  <files>
    scripts/generate-uk-postcodes.ts
    data/uk_postcode_sectors.csv
  </files>
  <action>
    Create `scripts/generate-uk-postcodes.ts` — a Node.js/TypeScript script that generates the UK postcode sector CSV for offline reference and verification. NOTE: The CSV is NOT consumed by the Edge Function in Task 2. Task 2's Edge Function uses the same compact area definitions and generates sectors programmatically at runtime.
...
<task type="auto">
  ...
  <action>
    ...
    This function generates postcode sectors programmatically and batch-inserts into territories table.

    Implementation:
    1. Embed ONLY the compact area definitions (~120 entries) as a TypeScript constant array.
       Each entry: `{ area: string, minDistrict: number, maxDistrict: number, region: string }`.
       Do NOT embed all 11,000+ generated sectors — that risks Edge Function timeout and is unmaintainable.
       Do NOT read from CSV file (Edge Functions can't read local files).

    2. Generate all sectors programmatically at runtime (same loop as generate-uk-postcodes.ts):
       For each area definition, loop from minDistrict to maxDistrict, producing `{area}{district}` entries.
    ...
    IMPORTANT: The Edge Function must be self-contained. Embed the ~120 compact area definitions (area, minDistrict, maxDistrict, region) and generate sectors at runtime. Do NOT embed all 11,000+ rows. The CSV file from Task 1 is for offline reference/verification only — it is NOT read by this Edge Function.
  </action>
  <verify>
    - Edge Function exists at `supabase/functions/seed-uk-postcodes/index.ts`
    - Contains compact area definitions (~120 entries), NOT 11,000+ embedded rows
    - Generates sectors programmatically at runtime from area definitions
...
3. `seed-uk-postcodes/index.ts` uses compact area definitions (~120 entries) and generates sectors programmatically for batch-insert
