...
<task type="auto">
  ...
  <action>
    EXECUTION ORDER: Create files in this sequence, validating each before proceeding:
    ...
    1. Create shared Stripe client `supabase/functions/_shared/stripe.ts`:
       ...
       - VALIDATE: Verify file compiles (no syntax errors, correct imports) before proceeding

    2. Create Edge Function `supabase/functions/stripe-connect/index.ts`:
       - MUST import from `../_shared/stripe.ts` (created in step 1)
  ...
  <verify>
    - Step 1: `_shared/stripe.ts` imports Stripe v14, exports stripe instance and STRIPE_WEBHOOK_SECRET — verify no syntax errors
    - Step 2: `stripe-connect/index.ts` imports from `../_shared/stripe.ts` and handles all 4 actions
    - Step 3: Test each action independently:
      - create_express_account: calls stripe.accounts.create with type 'express', country 'GB'
      - create_payment_intent: inserts into payments table with status 'pending'
      - create_customer: updates clients table with stripe_customer_id
      - check_account_status: retrieves account and returns status fields
  </verify>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    1. **Validate platform Connect account exists in test mode:**
       - Visit Stripe Dashboard -> Connect -> confirm Connect is enabled
       - Confirm the account is in TEST mode (toggle in dashboard header)
       - This is REQUIRED before any Express account can be created

    2. **Validate environment variables are set:**
       - Run: `supabase secrets list` — verify STRIPE_SECRET_KEY and STRIPE_WEBHOOK_SECRET are present
       - Confirm STRIPE_SECRET_KEY starts with `sk_test_`
       - Confirm STRIPE_WEBHOOK_SECRET starts with `whsec_`
       - Verify in Stripe Dashboard -> Developers -> API keys that test key matches

    3. **Review Edge Function code:**
       - supabase/functions/_shared/stripe.ts uses Stripe v14 for Deno compatibility
       - supabase/functions/stripe-connect/index.ts handles all 4 actions
       - supabase/functions/stripe-webhooks/index.ts handles all 4 event types

    4. **Validate webhook endpoint configured:**
       - Stripe Dashboard -> Developers -> Webhooks shows endpoint for account.updated and payment_intent.succeeded events
  </how-to-verify>
