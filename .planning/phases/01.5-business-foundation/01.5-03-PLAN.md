...
must_haves:
  truths:
    ...
    - "Auto-assignment algorithm ranks medics by distance, utilization, qualifications and updates booking status to 'assigned'"
...
<task type="auto">
  ...
  <action>
    ...
    5. **Auto-assign top candidate:**
       ...
       - **Update booking status to 'assigned'** after successful medic assignment
       - If top score < 50: set requires_manual_approval = true, approval_reason = "Low auto-match score", status remains 'pending' (NOT 'assigned')
       - If no candidates found: set requires_manual_approval = true, approval_reason = "No available medics", status remains 'pending'
  ...
  <verify>
    ...
    - Low-score matches trigger manual approval and booking status stays 'pending'
    - Successful assignment (score >= 50) updates booking status to 'assigned'
