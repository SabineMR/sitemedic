...
const { data: message, error: msgError } = await supabase
  ...
  .single();
```
- Update conversation metadata (last_message_at, last_message_preview) -- preview should show file icon emoji + filename: `last_message_preview: `📎 ${file.name}`.substring(0, 100)`
