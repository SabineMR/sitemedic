...
Output: supabase/migrations/134_org_logos_bucket.sql with bucket creation and 7 storage RLS policies.
</objective>
...
@.planning/ROADMAP.md
@.planning/STATE.md
@supabase/migrations/131_emergency_recordings_bucket.sql (structural reference for storage bucket migration format ONLY -- policy count differs)
</context>
...
<task type="auto">
  ...
  IMPORTANT: Do NOT use migration 131 as the policy count template -- it has only 3 policies. This migration requires exactly 7 separate policies as specified below. The context file is loaded for structural reference only (bucket INSERT format, comment conventions), not policy count.

  The migration must contain the following sections, in this order:
  ...
  **2. EXACTLY 7 RLS POLICIES (2 INSERT, 1 SELECT, 2 UPDATE, 2 DELETE):**

  Policy 1 -- Org admins INSERT (with folder check):
  ```sql
  ...
  Policy 2 -- Platform admins INSERT (no folder check -- their org_id is NULL):
  ```sql
  ...
  Policy 3 -- Anyone SELECT (public bucket):
  ```sql
  ...
  Policy 4 -- Org admins UPDATE (with folder check):
  ```sql
  ...
  Policy 5 -- Platform admins UPDATE (no folder check):
  ```sql
  ...
  Policy 6 -- Org admins DELETE (with folder check):
  ```sql
  ...
  Policy 7 -- Platform admins DELETE (no folder check):
  ```sql
  ...
  **3. SUMMARY comment block** at the bottom following migration 131 convention.

  Total: exactly 7 policies (2 INSERT, 1 SELECT, 2 UPDATE, 2 DELETE).

  IMPORTANT: Postgres arrays are 1-indexed -- use `[1]` not `[0]` in foldername checks.
  ...
  5. File contains exactly 7 `CREATE POLICY` statements
