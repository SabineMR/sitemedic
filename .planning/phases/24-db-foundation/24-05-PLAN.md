...
Output: Verification that all migration files are complete and correct. No files created or modified -- this is a validation-only plan.
</objective>
...
<task type="auto">
  ...
  <files></files>
  <action>
  This documentation goes into the SUMMARY for plan 24-05 (not a separate file). When writing the 24-05 SUMMARY, include a "Legacy Org Behaviour" section that documents:

  **For downstream phases (25-31) to reference:**
  ...
  <verify>
  This is documentation that goes into the SUMMARY file. Verify by reading the SUMMARY after it's written and confirming all 6 points are present.
  </verify>
  <done>
