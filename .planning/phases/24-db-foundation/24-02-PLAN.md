...
<task type="auto">
  ...
  **5. RLS POLICIES (6 total, following migration 118 pattern):**
  ...
  **7. SUMMARY comment block** at the bottom following migration 118 convention.
  ...
  <done>
  Migration 132 file exists with: org_branding table (6 columns + timestamps), org_id index, backfill carrying company_name from organizations.name, RLS enabled with 5 policies (org SELECT, org admin UPDATE, platform admin SELECT/INSERT/UPDATE). Ready to apply.
  </done>
</task>
...
- Backfill carries `organizations.name` into `company_name`
