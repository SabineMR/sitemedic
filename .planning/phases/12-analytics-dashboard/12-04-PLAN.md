...
must_haves:
  ...
  key_links:
    - from: "web/lib/queries/admin/analytics.ts"
      ...
      via: "supabase .from() query"
      pattern: "from\\('auto_schedule_logs'\\)"
    - from: "web/lib/queries/admin/analytics.ts"
      to: "medic_alerts"
...
- Query `auto_schedule_logs` table
- Filter: `created_at >= NOW() - 12 weeks`
- Join to bookings table to filter by org_id: `.eq('booking_id', bookingId)` won't work directly. Instead, first fetch booking IDs for this org, then filter logs.
- Actually, since Supabase client can't do subqueries easily, use this approach:
  1. Fetch all auto_schedule_logs with `created_at >= 12 weeks ago`
  2. Fetch booking IDs for org: `supabase.from('bookings').select('id').eq('org_id', orgId)`
  3. Filter logs client-side by booking_id membership (dataset is small, 12 weeks of logs)
...
- Reuse data from `fetchMedicsWithMetrics` pattern (import from medics.ts)
- Actually, to avoid circular dependency, duplicate the minimal query:
...
- Filter: `.eq('org_id', orgId)` and `.gt('travel_bonus', 0)` (travel_bonus > 0 indicates out-of-territory)
- Select: `id, medic_id, site_postcode, shift_date, travel_bonus, room_board`
...
- Calculate total_extra_cost = travel_bonus + room_board per booking
