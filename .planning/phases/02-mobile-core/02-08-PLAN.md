...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  9. Test airplane mode: Turn on airplane mode, repeat ALL above workflows -- every feature must work with NO network
...
- Verify offline capability: Enable airplane mode on device, then test all core workflows (treatment logging, near-miss capture, worker add, daily check). All must complete successfully with zero network access. Data must persist locally and queue for sync when connectivity returns.
</verification>
...
- 100% offline operation (airplane mode test confirms all workflows function without network)
</success_criteria>
