...
must_haves:
  ...
  key_links:
    ...
    - from: "mobile/app/treatment/new.tsx"
      ...
      via: "Import PhotoCapture from Plan 02 for treatment photos"
      pattern: "import.*PhotoCapture.*from.*components/forms/PhotoCapture"
    - from: "mobile/app/treatment/new.tsx"
...
<task type="auto">
  ...
  **Critical imports from prior plans:**
  - Import PhotoCapture from `../../components/forms/PhotoCapture` (built in Plan 02 -- provides camera/gallery photo capture with compression)
  - Import SignaturePad from `../../components/forms/SignaturePad` (built in Plan 02)
  - Import WorkerSearchPicker from `../../components/forms/WorkerSearchPicker` (built in Plan 03 -- provides worker search with quick-add)
  - Import BottomSheetPicker from `../../components/ui/BottomSheetPicker` (built in Plan 01)
  - Import LargeTapButton from `../../components/ui/LargeTapButton` (built in Plan 01)
  - Import { INJURY_TYPES } from `../../services/taxonomy/injury-types` (built in Plan 01)
  - Import { TREATMENT_TYPES } from `../../services/taxonomy/treatment-types` (built in Plan 01)
  - Import { OUTCOME_CATEGORIES } from `../../services/taxonomy/outcome-categories` (built in Plan 01)
  - Import { useAutoSave, AutoSaveIndicator } from `../../components/forms/AutoSaveForm` (built in Task 1 above)
  - Import BodyDiagramPicker from `../../components/forms/BodyDiagramPicker` (built in Task 1 above)
  ...
  **[id].tsx (Treatment View/Edit):**
  ...
  Verify PhotoCapture is imported from `components/forms/PhotoCapture` (Plan 02).
  Verify SignaturePad is imported from `components/forms/SignaturePad` (Plan 02).
  Verify WorkerSearchPicker is imported from `components/forms/WorkerSearchPicker` (Plan 03).
  </verify>
  <done>Full treatment workflow captures all clinical data in single scrollable form with auto-save. Reference numbers generated. RIDDOR auto-flagging on injury selection. Photos (up to 4) and digital signature supported. Draft/complete status tracking. All cross-plan components (PhotoCapture, SignaturePad, WorkerSearchPicker) explicitly imported.</done>
</task>
...
- PhotoCapture imported from Plan 02's `components/forms/PhotoCapture`
- SignaturePad imported from Plan 02's `components/forms/SignaturePad`
</verification>
