...
must_haves:
  truths:
    ...
    - "getLocationLabel('sporting_events') returns 'Pitch / Ground' (not 'Venue')"
    - "getEventLabel('sporting_events') returns 'Club' (not 'Event')"
  ...
  artifacts:
    ...
    - path: "services/taxonomy/vertical-outcome-labels.ts"
      provides: "getPatientLabel returns 'Player', getLocationLabel returns 'Pitch / Ground', getEventLabel returns 'Club' for sporting_events"
      contains: "'Player'"
...
<objective>
Wire football-specific terminology and certification types into the platform taxonomy. Three files need targeted updates: the patient label, location label, and event label all change for sporting_events in vertical-outcome-labels.ts; the org-labels.ts sporting_events entry updates to football terminology; and four FA-specific cert types are added to certification-types.ts with the sporting_events CertCategory added to the union.
...
<task type="auto">
  <name>Task 2: Update patient, location, and event labels for football terminology</name>
  <files>
    services/taxonomy/vertical-outcome-labels.ts
    web/lib/org-labels.ts
  </files>
  <action>
    **File 1: services/taxonomy/vertical-outcome-labels.ts**
    ...
    Read the full file. Make three targeted label changes for the `sporting_events` vertical:

    **Change 1 — getPatientLabel:** Locate the `sporting_events` entry in the `labels` object (currently `'Participant'`). Change it to `'Player'`:
    ```typescript
    // Before:
    ...
    **Change 2 — getLocationLabel:** Locate the `sporting_events` entry in the `getLocationLabel` labels object (currently `'Venue'`). Change it to `'Pitch / Ground'`:
    ```typescript
    // Before:
    ...
    **Change 3 — getEventLabel:** Locate the `sporting_events` entry in the `getEventLabel` labels object (currently `'Event'`). Change it to `'Club'`:
    ```typescript
    // Before:
  ...
  <verify>
    Confirm getPatientLabel change:
    ```
    grep -n "sporting_events" services/taxonomy/vertical-outcome-labels.ts
    ```
    Must show 'Player' on the getPatientLabel line (not 'Participant').

    Confirm getLocationLabel change:
    ```
    grep -n "Pitch / Ground" services/taxonomy/vertical-outcome-labels.ts
    ```
    Must return at least 1 match.

    Confirm getEventLabel change:
    ```
    grep -n "'Club'" services/taxonomy/vertical-outcome-labels.ts
    ```
    Must return at least 1 match (the sporting_events entry in getEventLabel).

    Confirm no old label values remain in the sporting_events entries:
    ```
    grep -n "'Venue'\|'Event'\|'Participant'" services/taxonomy/vertical-outcome-labels.ts
    ```
    Must return zero matches on lines that contain 'sporting_events' (other verticals may still have Venue/Event values — check only the sporting_events context).

    Confirm org-labels update:
    ```
    grep -n "Pitch / Ground" web/lib/org-labels.ts
    ```
    Must return at least 1 match in LABEL_MAP.

    ```
    grep -n "personSingular.*Player\|Player.*personSingular" web/lib/org-labels.ts
    ```
    Confirms update.

    ```
    grep -n "Participant\|Stadium" web/lib/org-labels.ts
    ```
    Must NOT appear in the sporting_events LABEL_MAP entry (confirm no leftover old values — other verticals may legitimately have these values).
  ...
  <done>
    ...
    useOrgLabels() for sporting_events returns personSingular: 'Player', personPlural: 'Players', locationTerm: 'Pitch / Ground', eventTerm: 'Club'.
    The treatment form section heading reads '1. Player Information' when on sporting_events vertical.
    No other verticals are affected.
  </done>
</task>
...
- `grep -n "'Player'" services/taxonomy/vertical-outcome-labels.ts` — at least 1 match on the sporting_events line in getPatientLabel
- `grep -n "Pitch / Ground" services/taxonomy/vertical-outcome-labels.ts` — 1 match (getLocationLabel sporting_events entry)
- `grep -n "'Club'" services/taxonomy/vertical-outcome-labels.ts` — 1 match (getEventLabel sporting_events entry)
...
<success_criteria>
Football terminology is correct throughout the platform: treatment form shows '1. Player Information', getLocationLabel returns 'Pitch / Ground', getEventLabel returns 'Club', web admin uses 'Player', 'Pitch / Ground', 'Club'. getRecommendedCertTypes('sporting_events') leads with ATMMiF. CertCategory accepts 'sporting_events' without TypeScript error. No other verticals are affected.
</success_criteria>
