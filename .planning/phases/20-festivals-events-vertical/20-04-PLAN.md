...
must_haves:
  truths:
    ...
    - "The medic profile cert types list shows FREC 3, IHCD Ambulance Aid, EFR for festivals vertical"
  artifacts:
...
<task type="auto">
  ...
  <action>
    **Step 0: Check and fix TypeScript Treatment type**
    ...
    Read `web/types/database.types.ts` (or wherever the Treatment type is defined — check imports in the treatment detail page). If `event_vertical` is missing from the Treatment row type, add it:
    ```typescript
    event_vertical: string | null;
    ```
    This column was added in migration 124 (Phase 18) but the TypeScript type may not have been regenerated. If the type already includes `event_vertical`, skip this step.
...
<task type="auto">
  ...
  <action>
    **FEST-05: Medic profile cert types should show FREC 3, IHCD Ambulance Aid, EFR for festivals vertical.**

    The function `getRecommendedCertTypes('festivals')` already exists in `web/types/certification.types.ts` (confirmed by research — returns FREC 3 first). The medic profile page at `web/app/medic/profile/page.tsx` currently imports `CERT_TYPE_METADATA` but does NOT call `getRecommendedCertTypes`. It displays existing certifications but has no "Recommended Certifications" section.
      ...
      <div className="bg-gray-800/50 border border-gray-700/50 rounded-2xl p-6">
        ...
        <div className="flex flex-wrap gap-2">
          {getRecommendedCertTypes(primaryVertical).slice(0, 6).map((certType) => {
            const meta = CERT_TYPE_METADATA[certType.id];
            const alreadyHeld = (medic.certifications || []).some(
              (c) => c.type === certType.id
            );
            return (
              <span
                key={certType.id}
              ...
              >
                {alreadyHeld && '✓ '}{meta?.label ?? certType.id}
              </span>
            );
          })}
        </div>
    ...
    **IMPORTANT:**
    - Check the exact shape returned by `getRecommendedCertTypes` — it returns `UKCertType[]` where each item has an `id` property. Read `web/types/certification.types.ts` to confirm the UKCertType shape.
    - If the return is a simple string array (not objects), adjust the `.map()` accordingly: use `certType` directly as the string key.
  ...
  <verify>
    1. Read back `web/app/medic/profile/page.tsx` and confirm:
       ...
       - FREC 3, IHCD Ambulance Aid, EFR would appear for festivals vertical
  ...
  <done>
    ...
    - For festivals vertical: FREC 3, IHCD Ambulance Aid, EFR appear prominently
...
4. Medic profile for festivals org shows FREC 3, IHCD Ambulance Aid, EFR in recommended certs section
...
- FEST-05 satisfied: Medic profile cert types list shows FREC 3, IHCD Ambulance Aid, EFR for festivals vertical
