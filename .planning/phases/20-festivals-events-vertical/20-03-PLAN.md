...
must_haves:
  ...
  key_links:
    ...
    - from: "supabase/functions/event-incident-report-generator/index.ts"
      to: "bookings table"
      via: "bookings(site_name, shift_date) join in .select()"
      pattern: "bookings"
---
...
<task type="auto">
  ...
  <action>
    ...
    **IMPORTANT: First, read `supabase/functions/riddor-f2508-generator/F2508Document.tsx` to understand the structural pattern (StyleSheet definition, Document/Page/View/Text layout, section grouping, footer).** Use it as the structural template for PurpleGuideDocument.tsx -- match the import style, StyleSheet organization, and section layout pattern.

    Then follow the F2508Document.tsx pattern exactly for imports:
    ```typescript
    ...
    **CRITICAL: The bookings table columns are `site_name` and `shift_date` (NOT `event_name`/`event_date`).** The treatment query in Task 2 joins `bookings(site_name, shift_date)`. The mapping must use these actual column names.
    ...
    export function mapTreatmentToPurpleGuide(treatment: any): PurpleGuideData {
      ...
      return {
        ...
        // Event name: use booking site_name (which is the venue/event name for festivals),
        // fall back to org company_name
        eventName: treatment.bookings?.site_name ?? treatment.org_settings?.company_name ?? 'Event',
        // Event date: use booking shift_date (the date of the event shift),
        // fall back to treatment_date
        eventDate: (treatment.bookings?.shift_date ?? treatment.treatment_date)
          ? new Date(treatment.bookings?.shift_date ?? treatment.treatment_date).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })
          : 'Date not recorded',
  ...
  <verify>
    ...
    4. Confirm PurpleGuideDocument.tsx follows the same structural pattern as F2508Document.tsx (StyleSheet, sections, footer)
    5. Confirm purple-guide-mapping.ts uses `treatment.bookings?.site_name` (NOT `event_name`) and `treatment.bookings?.shift_date` (NOT `event_date`)
    6. Confirm purple-guide-mapping.ts handles null vertical_extra_fields without crashing
    7. Confirm migration 125 creates the storage bucket
  </verify>
  <done>
    ...
    - PurpleGuideDocument.tsx follows F2508Document.tsx structural pattern (confirmed by reading template first)
    - purple-guide-mapping.ts exports mapTreatmentToPurpleGuide with graceful null handling
    - purple-guide-mapping.ts uses bookings.site_name for event name and bookings.shift_date for event date
...
<task type="auto">
  ...
  <action>
    ...
    // 2. Fetch treatment with joins (including bookings for event name/date)
    const { data: treatment, error: fetchError } = await supabase
      .from('treatments')
      .select(`*, medics(first_name, last_name), org_settings!treatments_org_id_fkey(company_name), workers(first_name, last_name), bookings(site_name, shift_date)`)
    ...
    **IMPORTANT:**
    ...
    - The join on `org_settings` uses the FK alias `treatments_org_id_fkey` -- confirmed by checking the F2508 generator pattern. The F2508 generator queries from `riddor_incidents` (not treatments), so its FK aliases differ. For treatments, use `org_settings!treatments_org_id_fkey(company_name)`.
    - The `bookings(site_name, shift_date)` join follows the standard Supabase FK convention -- treatments.booking_id references bookings(id), so `bookings(...)` resolves automatically via the FK.
  ...
  <verify>
    1. Read back `index.ts` and confirm:
       ...
       - Treatment fetch with joins exists, including `bookings(site_name, shift_date)`
    ...
    4. Confirm the select string includes: `*, medics(...), org_settings!treatments_org_id_fkey(...), workers(...), bookings(site_name, shift_date)`
  </verify>
  <done>
    - 501 stub replaced with full PDF generation flow
    - Treatment fetched from DB with medics, org_settings, workers, AND bookings joins
    - bookings join provides site_name (event name) and shift_date (event date) for the PDF
...
1. All 5 files exist and have correct content:
   ...
   - purple-guide-mapping.ts: mapTreatmentToPurpleGuide function (uses bookings.site_name, bookings.shift_date)
   - index.ts: full PDF flow (no 501), select includes bookings(site_name, shift_date)
...
5. purple-guide-mapping.ts handles null bookings gracefully (falls back to org_settings.company_name / treatment_date)
</verification>

<success_criteria>
- FEST-06 backend satisfied: event-incident-report-generator Edge Function produces a Purple Guide PDF
- PDF includes event name (from bookings.site_name) and event date (from bookings.shift_date)
...
- Graceful handling of missing data (defaults, not crashes) -- including null bookings
