...
must_haves:
  truths:
    ...
    - "When a company's required documents expire, the company is automatically suspended from quoting and receives an email notification"
    - "Approved companies display a 'Verified' badge (green shield icon with checkmark) that is visible on the admin queue, detail page, and reusable for future marketplace profile pages"
  artifacts:
    ...
    - path: "web/components/marketplace/VerifiedBadge.tsx"
      provides: "Reusable verified badge component showing green shield with checkmark for verified companies"
...
<task type="auto">
  ...
  <files>web/app/platform/verification/page.tsx, web/app/platform/verification/[id]/page.tsx, web/lib/marketplace/admin-actions.ts, web/components/marketplace/VerifiedBadge.tsx</files>
  <action>
**File 0: `web/components/marketplace/VerifiedBadge.tsx`**

Create a reusable VerifiedBadge component. This is used on the admin verification queue, the company detail page, and will be reused on future marketplace company profile pages.

```typescript
// Props: { status: VerificationStatus; size?: 'sm' | 'md' | 'lg' }
// Renders:
// - 'verified': Green shield with checkmark (ShieldCheck from lucide-react) + "Verified" text
// - 'cqc_verified': Blue shield (ShieldCheck) + "CQC Verified" text (auto-verified, awaiting admin)
// - 'pending': Yellow clock icon (Clock) + "Pending Review" text
// - 'info_requested': Amber alert icon (AlertTriangle) + "Info Requested" text
// - 'rejected': Red X icon (XCircle) + "Rejected" text
// - 'suspended': Red shield (ShieldX or ShieldOff) + "Suspended" text
//
// Uses Tailwind classes for colors: green-600, blue-600, yellow-600, amber-600, red-600
// size='sm' for table rows (inline badge), 'md' for detail pages, 'lg' for profile headers
```

Import and use this component in both verification/page.tsx (queue table rows) and verification/[id]/page.tsx (detail header).
...
Run `grep "VerifiedBadge" web/components/marketplace/VerifiedBadge.tsx` — should match the export.
Run `grep "VerifiedBadge" web/app/platform/verification/page.tsx` — should match the import (badge used in queue table).
...
serve(async (req) => {
    ...
    for (const company of companies) {
      ...
      try {
        ...
        if (provider.registrationStatus !== 'Registered') {
          ...
          // Send suspension email to company
          ...
          if (companyDetail?.company_email) {
            // Use Resend (follow existing email pattern from web/lib/email/templates.ts)
            // Subject: "Your marketplace registration has been suspended"
            // Body: Include company name, suspension reason (CQC status change), and instructions to contact support
            await fetch('https://api.resend.com/emails', {
              ...
              body: JSON.stringify({
                from: Deno.env.get('RESEND_FROM_EMAIL') || 'noreply@sitemedic.co.uk',
                to: companyDetail.company_email,
                subject: `${companyDetail.company_name}: Marketplace registration suspended - CQC status change`,
                html: `<p>Your marketplace registration for <strong>${companyDetail.company_name}</strong> has been suspended because your CQC registration status changed to <strong>${provider.registrationStatus}</strong>.</p><p>You cannot submit quotes until this is resolved. Please contact CQC to reinstate your registration, then contact SiteMedic support.</p>`,
              }),
            });
          }
          ...
          if (activeBookings?.length) {
            // Log for admin — these need manual review (NOT auto-cancel per CONTEXT decision)
            console.warn(`ADMIN ALERT: Company ${company.company_name} (${company.id}) suspended - ${activeBookings.length} active marketplace bookings need review`);
            // Future: Create admin notification/alert record in notifications table
          }
  ...
  if (expiredDocs?.length) {
    ...
    for (const companyId of companyIds) {
      ...
      if (hasExpiredRequired) {
        ...
        // Send expiry suspension email to company
        ...
        if (suspendedCompany?.company_email) {
          await fetch('https://api.resend.com/emails', {
            ...
            body: JSON.stringify({
              from: Deno.env.get('RESEND_FROM_EMAIL') || 'noreply@sitemedic.co.uk',
              to: suspendedCompany.company_email,
              subject: `${suspendedCompany.company_name}: Marketplace registration suspended - expired documents`,
              html: `<p>Your marketplace registration for <strong>${suspendedCompany.company_name}</strong> has been suspended because the following compliance documents have expired: <strong>${companyExpiredTypes.join(', ')}</strong>.</p><p>Please upload updated documents to restore your quoting ability.</p>`,
            }),
          });
        }

        // Flag active marketplace bookings for admin review (same as CQC suspension)
        ...
        if (affectedBookings?.length) {
          console.warn(`ADMIN ALERT: Company ${suspendedCompany?.company_name} (${companyId}) suspended due to expired docs - ${affectedBookings.length} active marketplace bookings need review`);
        }
  ...
  if (expiringDocs?.length) {
    ...
    const expiringByCompany = new Map<string, typeof expiringDocs>();
    ...
    for (const [companyId, docs] of expiringByCompany) {
      ...
      if (comp?.company_email) {
        const docList = docs.map(d => `${d.document_type} (expires ${d.expiry_date})`).join(', ');
        await fetch('https://api.resend.com/emails', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${Deno.env.get('RESEND_API_KEY')}`,
          ...
          body: JSON.stringify({
            from: Deno.env.get('RESEND_FROM_EMAIL') || 'noreply@sitemedic.co.uk',
            to: comp.company_email,
            subject: `${comp.company_name}: Compliance documents expiring soon`,
            html: `<p>The following compliance documents for <strong>${comp.company_name}</strong> will expire within 30 days: <strong>${docList}</strong>.</p><p>Please upload updated documents before they expire to avoid suspension from quoting.</p>`,
          }),
        });
      }
    }
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    4. Verify the queue shows any pending marketplace companies. NOTE: If Plan 02 has not been executed yet, seed test data by inserting a marketplace_companies row directly via Supabase SQL editor with verification_status='pending' and a matching compliance_documents row. Alternatively, execute Plan 02 first to create a test company via the registration wizard.
