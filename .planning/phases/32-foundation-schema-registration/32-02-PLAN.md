...
must_haves:
  truths:
    ...
    - "A client (company or individual) can register on the marketplace via a lightweight page that toggles marketplace_enabled=true on their clients row — no heavy wizard, low friction"
  artifacts:
    ...
    - path: "web/app/api/marketplace/client-register/route.ts"
      provides: "POST endpoint to toggle marketplace_enabled=true on clients table"
      exports: ["POST"]
    - path: "web/app/marketplace/client-register/page.tsx"
      provides: "Lightweight client marketplace registration page"
      min_lines: 30
  key_links:
    ...
    - from: "web/app/api/marketplace/client-register/route.ts"
      to: "clients table"
      via: "Supabase update marketplace_enabled"
...
<task type="auto">
  <name>Task 1a: Create Zustand registration store and CQC verify API</name>
  <files>web/stores/useMarketplaceRegistrationStore.ts, web/app/api/marketplace/cqc-verify/route.ts</files>
  <action>
...
Check both files compile: `npx tsc --noEmit` in the web directory.
  </verify>
  <done>Zustand store manages wizard state with step navigation, pre-fill from existing org, and document tracking. CQC verify API proxies to CQC public API and returns verification result.</done>
</task>

<task type="auto">
  <name>Task 1b: Create registration and document upload API routes</name>
  <files>web/app/api/marketplace/register/route.ts, web/app/api/marketplace/upload-document/route.ts</files>
  <action>
**File 1: `web/app/api/marketplace/upload-document/route.ts`**
...
**File 2: `web/app/api/marketplace/register/route.ts`**
...
Run `grep "service.*role\|serviceRole\|SUPABASE_SERVICE_ROLE" web/app/api/marketplace/register/route.ts` — should match service-role client usage.
Check both files compile: `npx tsc --noEmit` in the web directory.
  </verify>
  <done>Document upload API handles multipart file upload to private compliance-documents bucket with ownership verification. Registration API creates marketplace_companies row with bidirectional crossover (existing org linking or new org creation) using service-role client.</done>
</task>
...
<task type="auto">
  <name>Task 3: Create lightweight client marketplace registration</name>
  <files>web/app/api/marketplace/client-register/route.ts, web/app/marketplace/client-register/page.tsx</files>
  <action>
**File 1: `web/app/api/marketplace/client-register/route.ts`**

POST endpoint for lightweight client marketplace registration. Accepts `{}` (no body needed — just toggles the flag for the authenticated user).

Implementation:
1. Authenticate the user via createClient()
2. Look up the user's client record (via user_id or org association)
3. If no client record exists: return 404 with "No client account found. Please sign up first."
4. Update `clients` table: `marketplace_enabled = true` for the matching row
5. Return `{ success: true, message: "Marketplace access enabled" }`

This is intentionally minimal per CONTEXT decision: "keep it lightweight for low friction; collect billing/company details later when they actually award a job and need to pay."

**File 2: `web/app/marketplace/client-register/page.tsx`**

Lightweight client marketplace registration page. Client component.

Layout:
- Use the same clean layout as the company registration (no dashboard sidebar)
- Heading: "Post Events on the Marketplace"
- Short description: "Register to post events needing medical cover. Verified medic companies will submit quotes for your review."
- Bullet points: "Free to post events", "Compare quotes from verified companies", "Pay only when you award a quote"
- Single CTA button: "Enable Marketplace Access"
- On click: call `/api/marketplace/client-register` POST
- On success: redirect to `/marketplace/events/new` (or show success toast with "You can now post events" and a "Post Your First Event" link)
- If already enabled: show "You already have marketplace access" with link to post an event

Use existing UI components (Button, Card). Keep it under 80 lines — this is intentionally lightweight.
  </action>
  <verify>
Run `ls web/app/marketplace/client-register/` — should list page.tsx.
Run `ls web/app/api/marketplace/client-register/` — should list route.ts.
Run `grep "marketplace_enabled" web/app/api/marketplace/client-register/route.ts` — should match the update.
  </verify>
  <done>Client marketplace registration is a single-page, single-click flow that toggles marketplace_enabled=true. No heavy wizard — clients can post events immediately after enabling. Billing details collected later at award time.</done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <what-built>Multi-step marketplace company registration wizard with CQC verification, document upload, and company creation. Also includes lightweight client marketplace registration. Includes Zustand store for wizard state, 3 company API routes, 1 client API route, and responsive UI.</what-built>
  <how-to-verify>
    ...
    10. Navigate to http://localhost:30500/marketplace/client-register
    11. Verify the lightweight client registration page renders
    12. Click "Enable Marketplace Access" and confirm marketplace_enabled is set to true on the clients row
  </how-to-verify>
...
8. Client marketplace registration page loads and toggles marketplace_enabled on the clients row
</verification>
...
- Client marketplace registration toggles marketplace_enabled=true
