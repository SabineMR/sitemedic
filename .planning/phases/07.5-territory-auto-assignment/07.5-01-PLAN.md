...
must_haves:
  truths:
    - "Territory metrics aggregate daily via pg_cron at 3 AM UTC"
    - "Hiring triggers fire when utilization >80% for 3+ weeks OR fulfillment <90%"
    - "Coverage gaps require minimum 10 bookings to avoid false positives"
    - "Territory metrics include rejection_rate, fulfillment_rate, utilization for each postcode sector"
  artifacts:
