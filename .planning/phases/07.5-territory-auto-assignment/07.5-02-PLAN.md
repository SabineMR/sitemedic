...
<task type="auto">
  ...
  <action>
**FIRST: Read the full `supabase/functions/auto-assign-medic-v2/index.ts` file.** Identify:
- The exact line where `findCandidateMedics` returns candidates (or where the candidate array is populated)
- The exact line where the scoring loop begins (where candidates are iterated to compute scores)
- The calendar check batch query MUST be inserted between these two points. Note both line numbers before proceeding.
...
- Executor confirmed the full file was read and identified the exact insertion point (line numbers noted for candidate retrieval end and scoring loop start)
