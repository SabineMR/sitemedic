...
1. **Calendar availability check:** Before scoring candidates, query the bookings table for each candidate medic to check if they have existing confirmed/in_progress bookings on the same shift_date. If a medic has a booking that overlaps the requested shift time (shift_start_time through shift_end_time), exclude them from the candidate pool. Use a single batch query with `.in('medic_id', candidateIds)` filter to avoid N+1.
...
- Calendar conflict query is batch (not N+1 loop)
