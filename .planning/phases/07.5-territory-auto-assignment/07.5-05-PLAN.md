...
<task type="auto">
  <name>Task 2: Integrate all components into territories page with tab navigation</name>
  <files>web/app/admin/territories/page.tsx</files>
  <action>
Modify page.tsx ONCE with all the following changes:
...
3. **Coverage Map Tab content:**
   - Import and render `CoverageAlerts` from `./coverage-alerts` (from Plan 04) above the map, pass `territories` prop, make collapsible (default expanded)
   - Import and render `HiringPanel` from `./hiring-panel` (from Plan 04) below CoverageAlerts, above the map, pass `territories` prop, make collapsible (default collapsed)
   - Render existing TerritoryMap and TerritoryList below
   - Layout order: CoverageAlerts -> HiringPanel -> TerritoryMap (50vh) -> TerritoryList (scrollable)

4. **Assignment Manager Tab content:**
   - Import AssignmentPanel from `./assignment-panel` (from Plan 03)
   - Render `<AssignmentPanel />` when activeTab === 'assignment'

5. **Territory detail panel state and click handlers:**
   - Add state: `const [selectedTerritory, setSelectedTerritory] = useState<TerritoryWithMetrics | null>(null)`
   - Pass `onTerritoryClick={setSelectedTerritory}` and `selectedTerritoryId={selectedTerritory?.id}` to TerritoryMap
   - Add `onRowClick={setSelectedTerritory}` to TerritoryList (or wrap rows with onClick), add cursor-pointer hover styling
   - Import TerritoryDetail from `./territory-detail` (from Task 1 of this plan)
   - Render `<TerritoryDetail territory={selectedTerritory} onClose={() => setSelectedTerritory(null)} />` conditionally
   - Panel slides in from right, overlaying on mobile (<lg) or side-by-side on desktop (>=lg)

6. **Change useTerritories refetchInterval** from 60000 to 300000 (5 minutes) to match Success Criteria #9.
  </action>
  <verify>
...
- Coverage Map tab shows CoverageAlerts, HiringPanel, map, and territory list in correct order
...
- Detail panel closes on close button or Escape key
- Responsive layout: overlay on mobile, side-by-side on desktop
...
- Assignment changes from detail panel update map and list via query invalidation
  ...
  <done>
Territories page fully integrated with: summary stats bar, tab navigation (Coverage Map / Assignment Manager), coverage alerts, hiring panel, territory detail side panel with click handlers, and 5-minute polling. All components from Plans 03 and 04 wired into cohesive tabbed interface.
  </done>
</task>

<task type="auto">
  <name>Task 3: Verify auto-assignment 95% success rate with simulated bookings</name>
