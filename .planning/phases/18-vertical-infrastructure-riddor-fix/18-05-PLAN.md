...
must_haves:
  ...
  artifacts:
    - path: "web/app/api/bookings/create/route.ts"
      provides: "Booking creation API with event_vertical persistence"
...
<objective>
Fix the one gap preventing VERT-06 (client booking vertical) from being complete: add eventVertical to the BookingRequest interface and wire it into the Supabase insert in the booking creation API.

Purpose: The booking form UI already captures eventVertical in ShiftConfig (confirmed in research — web/components/booking/shift-config.tsx lines 90, 94-101). The payment flow carries it through sessionStorage. The gap is purely at the API layer: BookingRequest has no eventVertical field and the Supabase insert omits event_vertical. The bookings.event_vertical column already exists (migration 123_booking_briefs.sql). This plan closes the gap with a targeted two-line change.
...
Output: Updated web/app/api/bookings/create/route.ts with eventVertical in BookingRequest and Supabase insert.
</objective>
...
<task type="auto">
  <name>Task 1: Add eventVertical to BookingRequest and wire it into the Supabase insert</name>
  <files>web/app/api/bookings/create/route.ts</files>
  <action>
    Read the full `web/app/api/bookings/create/route.ts` first (already read during planning — 197 lines).
    ...
    **Change 2: Add event_vertical to the Supabase insert.**
    ...
    These are the only two changes needed. Do NOT modify any other logic — the credit limit check, urgency premium validation, email notification, and response shape are all unchanged.

    The `bookings.event_vertical` column already exists (confirmed from migration 123_booking_briefs.sql — it has `event_vertical TEXT`). The value is optional on the client side so `?? null` is the correct default.
  </action>
  <verify>
  ...
  <done>BookingRequest interface has eventVertical?: string. The Supabase insert includes event_vertical: body.eventVertical ?? null. TypeScript compiles cleanly. A booking created with an eventVertical in the request body will now persist that value to bookings.event_vertical.</done>
</task>
...
1. API interface: `grep "eventVertical" web/app/api/bookings/create/route.ts` → 2 matches
2. API insert: `grep "event_vertical" web/app/api/bookings/create/route.ts` → 1 match
3. Admin vertical selector: `grep "toggleVertical" web/app/admin/settings/page.tsx` → 1+ matches
4. Booking form: `grep "eventVertical" web/components/booking/shift-config.tsx` → 2+ matches
...
- Supabase insert includes event_vertical: body.eventVertical ?? null
- bookings.event_vertical is populated when a client selects an event type during booking creation
