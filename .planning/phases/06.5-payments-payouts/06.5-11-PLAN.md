...
depends_on: []
files_modified:
  - supabase/migrations/026_payslip_schema_fix.sql
  - supabase/migrations/027_late_payment_cron_auth_fix.sql
autonomous: true
gap_closure: true
...
must_haves:
  ...
  key_links:
    ...
    - from: "supabase/migrations/027_late_payment_cron_auth_fix.sql"
...
<task type="auto">
  ...
  <files>supabase/migrations/026_payslip_schema_fix.sql</files>
  <action>
    Create migration `026_payslip_schema_fix.sql` that does three things:
    ...
    **Trigger execution order (cross-table cascade):**
    ...
    2. `trigger_generate_payslip_pdf` (this migration 026) fires `AFTER INSERT ON payslips` -- calls the Edge Function via pg_net.
    PostgreSQL executes cascading triggers correctly: when trigger 024's INSERT INTO payslips fires, it triggers 026's AFTER INSERT on payslips within the same transaction. No alphabetical ordering concern exists because these triggers live on separate tables (timesheets vs payslips).
  ...
  <verify>
    ...
    Run: `grep -c 'payslip_reference\|vault.decrypted_secrets\|generate-payslip-pdf\|trigger_generate_payslip_pdf' supabase/migrations/026_payslip_schema_fix.sql` should return 4+

    Verify trigger cascade chain (both triggers exist with correct timing on their respective tables):
    Run: `grep -A 3 'CREATE TRIGGER trigger_generate_payslip' supabase/migrations/024_payslip_generation.sql supabase/migrations/026_payslip_schema_fix.sql`
    Expected: 024 shows `AFTER UPDATE ON timesheets`, 026 shows `AFTER INSERT ON payslips` -- confirming cross-table cascade.
  </verify>
  <done>
...
<task type="auto">
  ...
  <files>supabase/migrations/027_late_payment_cron_auth_fix.sql</files>
  <action>
    Create migration `027_late_payment_cron_auth_fix.sql` that:
  ...
  <verify>
    ...
    Run: `grep -c 'vault.decrypted_secrets' supabase/migrations/027_late_payment_cron_auth_fix.sql` should return 6 (project_url x3 + service_role_key x3)
    Run: `grep -c 'current_setting' supabase/migrations/027_late_payment_cron_auth_fix.sql` should return 0

    Verify the cron job is properly scheduled (SQL structure is valid):
    Run: `grep 'SELECT cron.schedule' supabase/migrations/027_late_payment_cron_auth_fix.sql` should confirm the schedule call exists.
    Run: `grep 'SELECT cron.unschedule' supabase/migrations/027_late_payment_cron_auth_fix.sql` should confirm the old job removal exists.
  </verify>
  <done>
...
1. **Payslip flow verification:** Confirm the chain: timesheet.payout_status='paid' -> trigger_generate_payslip_on_payout (024) creates payslip row -> trigger_generate_payslip_pdf (026) calls Edge Function -> Edge Function reads payslip_reference (now exists) -> PDF uploaded -> pdf_url updated.
...
2. **Auth pattern consistency:** Verify all 3 cron jobs use vault.decrypted_secrets:
   ...
   - `grep -r 'vault.decrypted_secrets' supabase/migrations/027_late_payment_cron_auth_fix.sql` (should match)
   - `grep -r 'current_setting' supabase/migrations/` (should match ONLY migration 020 which is superseded by 027)

3. **Migration numbering:** Confirm sequential: 024 -> 025 -> 026 -> 027 with no gaps or duplicates.
</verification>
