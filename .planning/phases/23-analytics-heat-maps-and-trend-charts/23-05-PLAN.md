...
must_haves:
  ...
  artifacts:
    ...
    - path: "web/components/analytics/AdminComplianceTrend.tsx"
      provides: "Recharts ComposedChart showing aggregate compliance across all orgs with min/max band"
...
<task type="auto">
  ...
  <action>
    **File 1: web/components/analytics/AdminComplianceTrend.tsx**
    ...
    IMPORTANT: Use `ComposedChart` (NOT `LineChart`) as the chart container. The `<Area>` element for the min/max band is only valid inside `<ComposedChart>` or `<AreaChart>` — placing it inside `<LineChart>` will silently fail. The `ComposedChart` API is structurally identical to `LineChart`, just a different root element.
    ...
    - ComposedChart (NOT LineChart) showing:
      - Shaded band: Two `<Area>` elements for min_score and max_score range (fillOpacity=0.1, stroke="none" for min, fill="#3B82F6" for max). Use the standard Recharts band pattern: one Area for max_score with fill, one Area for min_score with fill matching background to "cut out" the lower region. Alternatively, use a single Area with a custom shape or the baseLine prop if simpler.
      - Main line: `<Line>` for avg_score (stroke="#3B82F6", strokeWidth=2)
  ...
  <verify>
    ...
    grep for 'ComposedChart' in AdminComplianceTrend.tsx — should appear in import AND as root chart element.
    grep for 'LineChart' in AdminComplianceTrend.tsx — should NOT appear (replaced by ComposedChart).
    grep for 'Area' in AdminComplianceTrend.tsx — should appear (min/max band).
    grep for 'Line' in AdminComplianceTrend.tsx — should appear (avg_score series).
  ...
  <done>Admin analytics page has a Compliance tab showing aggregate trend chart (using ComposedChart with Line + Area for min/max band) and top/bottom orgs table. Platform admins can compare compliance performance across all organisations.</done>
</task>

</tasks>

<verification>
1. AdminComplianceTrend uses ComposedChart (NOT LineChart) with Line for avg_score and Area for min/max range band
...
- Aggregate trend chart shows average score across all orgs with min-max range shading using ComposedChart
