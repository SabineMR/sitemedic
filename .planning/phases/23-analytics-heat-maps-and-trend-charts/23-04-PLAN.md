...
must_haves:
  ...
  artifacts:
    ...
    - path: "web/app/(dashboard)/analytics/compliance/page.tsx"
      provides: "Org dashboard compliance trends page with both charts"
      min_lines: 30
  key_links:
...
<objective>
Create the org-level compliance score trend chart and incident frequency trend chart on the org dashboard. Both charts show 12 months of weekly data points using Recharts.
...
Output: ComplianceScoreChart + IncidentFrequencyChart components, compliance-history query hooks, compliance dashboard page.
</objective>
...
<task type="auto">
  <name>Task 2: Create ComplianceScoreChart + IncidentFrequencyChart + compliance page</name>
  ...
  <action>
    ...
    **File 3: web/app/(dashboard)/analytics/compliance/page.tsx**
    ...
    - Add a link/breadcrumb to navigate back to heat-map: "View Heat Map" text link

    Note: The DashboardNav link from 23-02 points to /analytics/heat-map. For the compliance page, the user navigates via /analytics/compliance. The isActive logic in DashboardNav should highlight "Analytics" for both paths (already handled by startsWith('/analytics') in 23-02).
  </action>
  <verify>
  ...
  <done>Compliance score trend chart shows weekly scores with red/amber thresholds. Incident frequency chart shows treatment and near-miss weekly counts. Both accessible at /analytics/compliance on the org dashboard.</done>
</task>
