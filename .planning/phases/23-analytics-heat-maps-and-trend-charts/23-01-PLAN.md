...
<task type="auto">
  <name>Task 1: Create migration 130 — drop redundant index + UNIQUE index + platform admin RLS policy on compliance_score_history</name>
  <files>supabase/migrations/130_compliance_score_history_index_and_rls.sql</files>
  <action>
    Create SQL migration file with three statements:

    1. DROP the redundant non-unique index created in migration 124 (it is fully subsumed by the new UNIQUE index and only adds write overhead):
    ```sql
    DROP INDEX IF EXISTS compliance_score_history_period_idx;
    ```

    2. UNIQUE INDEX for upsert onConflict:
    ```sql
    ...
    3. Platform admin SELECT policy (table was created in migration 124 but has NO platform admin policy):
    ```sql
  ...
  <verify>
    ...
    File contains DROP INDEX IF EXISTS compliance_score_history_period_idx.
  ...
  <done>Migration 130 drops the redundant plain index, creates the unique index required for upsert, and creates the platform admin RLS policy required for plan 23-05.</done>
</task>
...
<verification>
1. Migration 130 file exists with exactly 3 SQL statements (DROP INDEX + UNIQUE INDEX + RLS POLICY)
...
7. Redundant non-unique index compliance_score_history_period_idx is dropped before creating the unique index
</verification>

<success_criteria>
- Migration 130 drops redundant plain index, creates UNIQUE INDEX on compliance_score_history(org_id, vertical, period_start), and creates platform admin SELECT policy
