...
import { RIDDORIncident } from '@/lib/queries/riddor';
...
/**
 * Export RIDDOR incidents to formatted PDF report
 *
 * Format: A4 landscape with header, formatted table, footer with page numbers
 * Table columns: Date, Worker, Injury, Body Part, Category, Confidence, Status, Deadline
 */
export function exportRIDDORIncidentsPDF(incidents: RIDDORIncident[]) {
  ...
  const tableData = incidents.map((i) => [
    i.detected_at ? format(new Date(i.detected_at), 'dd/MM/yyyy') : '',
    i.workers ? `${i.workers.first_name} ${i.workers.last_name}` : 'Unknown',
    i.treatments?.injury_type ? i.treatments.injury_type.replace(/-/g, ' ') : '',
    i.treatments?.body_part ? i.treatments.body_part.replace(/_/g, ' ') : '',
    i.category ? i.category.replace(/_/g, ' ') : '',
    i.confidence_level || '',
    i.status || '',
    i.deadline_date ? format(new Date(i.deadline_date), 'dd/MM/yyyy') : '',
  ]);
  ...
  autoTable(doc, {
    ...
    columnStyles: {
      0: { cellWidth: 25 }, // Date
      1: { cellWidth: 40 }, // Worker
      2: { cellWidth: 40 }, // Injury
      3: { cellWidth: 30 }, // Body Part
      4: { cellWidth: 40 }, // Category
      5: { cellWidth: 25 }, // Confidence
      6: { cellWidth: 22 }, // Status
      7: { cellWidth: 25 }, // Deadline
    },
    didDrawPage: (data) => {
      // Footer with page number
      const pageCount = doc.getNumberOfPages();
