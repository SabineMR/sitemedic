/**
 * Event Fan-Out: New Event Notifications (web-marketplace)
 * Phase 38: Notifications & Alerts — Plan 03
 *
 * Mirror of web/lib/marketplace/event-fan-out.ts — inline re-implementation
 * because web-marketplace cannot import from the web/ package.
 *
 ...
 *
 * IMPORTANT: Call fire-and-forget: void fanOutNewEventNotifications(...)
 */
...
// =============================================================================
// Private: service-role Supabase client
// =============================================================================
...
// =============================================================================
// Private: SMS helper (mirrors web/lib/marketplace/sms.ts)
// =============================================================================

async function sendSMSFanOut(to: string, body: string): Promise<void> {
  const accountSid = process.env.TWILIO_ACCOUNT_SID;
  const authToken = process.env.TWILIO_AUTH_TOKEN;
  const fromNumber = process.env.TWILIO_PHONE_NUMBER;

  if (!accountSid || !authToken || !fromNumber) {
    console.log('[SMS DEV MODE] Would send to:', to, '|', body);
    return;
  }
...
// =============================================================================
// Notification type constant (mirrors web/lib/marketplace/notification-types.ts)
// =============================================================================

const NEW_EVENT_TYPE = 'new_event' as const;
...
export async function fanOutNewEventNotifications(
  ...
  try {
    ...
    const adminUserIds = companies
      .map((c: { admin_user_id: string | null }) => c.admin_user_id)
    ...
    // 5. Fan out — individual try/catch per company
      ...
      try {
        ...
        if (company.admin_user_id) {
          try {
            ...
            console.error(
              `[FanOut] Dashboard notification failed for company ${company.id}:`,
              notifErr
            );
          }
        }
        ...
        if (
          ...
          try {
            // Simple daily cap check: count new_event notifications sent today
